...
# Initialize session state for display states
if 'display_states' not in st.session_state:
    st.session_state['display_states'] = {}
    ...
    for table in tables:
        ...
        if df is not None:
            if not df.empty:
                ...
                columns = [col for col in df.columns if col not in ['date', 'hour', 'minute', 'id']]

                for column in columns:
                    if st.button(f'Show {column} graph for {table}'):
                        st.session_state['display_states'][f'{table}_{column}'] = True

                    if st.session_state['display_states'].get(f'{table}_{column}', False):
